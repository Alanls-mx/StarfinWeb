...
function getAuthContext(req: express.Request): { token: string; userId: string; role: 'user' | 'admin' } | null {
  const token = getAuthToken(req);
  if (!token) return null;
  if (token === auth.demoToken) return { token, userId: demoUser.id, role: 'user' };
  if (token === auth.adminToken) return { token, userId: demoUser.id, role: 'admin' };
  const session = findSession(token);
  if (session) return { token, userId: session.userId, role: session.role };
  return null;
}
...
app.post('/api/plugin-auth/verify', (req, res) => {
  ...
  const license = getLicensesForUser(keyRecord.userId).find(
...
app.delete('/api/admin/categories', (req, res) => {
  ...
  const name = typeof req.body?.name === 'string' ? req.body.name.trim() : '';
  const idx = categories.findIndex((c) => c === (name as any));
  if (idx === -1) {
...
app.put('/api/admin/smtp', (req, res) => {
  ...
  const body = req.body ?? {};
  if (typeof body.enabled === 'boolean') smtpConfig.enabled = body.enabled;
  if (typeof body.host === 'string') smtpConfig.host = body.host.trim();
  if (typeof body.port === 'number') smtpConfig.port = body.port;
  if (typeof body.secure === 'boolean') smtpConfig.secure = body.secure;
  if (typeof body.user === 'string') smtpConfig.user = body.user.trim();
  if (typeof body.pass === 'string' && body.pass && body.pass !== '********') smtpConfig.pass = body.pass;
  if (typeof body.fromName === 'string') smtpConfig.fromName = body.fromName.trim();
  if (typeof body.fromEmail === 'string') smtpConfig.fromEmail = body.fromEmail.trim();
  res.json({ ok: true });
});
...
app.put('/api/admin/integrations', (req, res) => {
  ...
  const body = req.body ?? {};
  if (typeof body.panelBaseUrl === 'string') integrationConfig.panelBaseUrl = body.panelBaseUrl.trim();
  if (typeof body.pluginAuthEndpoint === 'string') integrationConfig.pluginAuthEndpoint = body.pluginAuthEndpoint.trim();
  if (typeof body.docsBaseUrl === 'string') integrationConfig.docsBaseUrl = body.docsBaseUrl.trim();
  res.json({ ok: true });
});
